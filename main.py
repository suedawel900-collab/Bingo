...
# Conversation states - REMOVED PHONE_NUMBER
AMOUNT, PAYMENT_METHOD, REFERENCE = range(3)
...
class IntegratedBingoGame:
    ...
    async def handle_deposit_amount(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            # Create payment request directly (no phone number)
            method = context.user_data.get('payment_method', 'telebirr')
            
            # Create payment request in database (without phone number)
            ...
            # Create payment request with empty phone number
...
# Payment conversation handler - UPDATED: Removed PHONE_NUMBER state
