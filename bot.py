...
async def play_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Get the latest game ID from webapp
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.get(f"{WEBAPP_URL}/api/current-game")
            data = resp.json()
            game_id = data.get('game_id', 1)
    except Exception as e:
        logger.error(f"Failed to get current game ID: {e}")
        game_id = 1   # fallback

    # Launch webapp with admin_id
    webapp_url = f"{WEBAPP_URL}/game?user_id={user.id}&game_id={game_id}&admin_id={ADMIN_USER_ID}"
